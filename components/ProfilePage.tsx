...
const ProfilePage: React.FC<ProfilePageProps> = ({ 
  ...
  const [localProjects, setLocalProjects] = useState<Project[]>(profile.projects);
  
  // --- Profile Edit & View Mode State ---
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, field: 'banner' | 'avatar') => {
      if (e.target.files && e.target.files[0]) {
          ...
          setUserProfile(prev => ({ ...prev, [field]: imageUrl }));
      }
  };

  const triggerBannerUpload = () => !isViewerMode && bannerInputRef.current?.click();
  const triggerAvatarUpload = () => !isViewerMode && avatarInputRef.current?.click();
    ...
    <div className="w-full max-w-[1600px] mx-auto pb-32 pt-6 px-6 lg:px-8 animate-in fade-in duration-500">
        ...
        <div className="relative rounded-3xl overflow-hidden bg-[#0a0a0a] border border-neutral-800 mb-8 group/header shadow-2xl">
            ...
            >
                ...
                {!isViewerMode && (
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/banner:opacity-100 transition-all duration-300 bg-black/20 backdrop-blur-sm">
                        <div className="flex items-center gap-2 text-white bg-black/60 border border-white/10 px-5 py-2.5 rounded-full shadow-xl transform translate-y-4 group-hover/banner:translate-y-0 transition-transform duration-300">
                            <Camera size={18} />
                         ...
                         <div className="flex items-center divide-x divide-white/10 bg-neutral-900/50 rounded-xl border border-white/5 backdrop-blur-md">
                             ...
                             <div className="px-5 py-3 text-center">
                                 <div className="text-lg font-black text-primary flex items-center justify-center gap-1 font-mono">
                                     {userProfile.gems.toLocaleString()} <Gem size={14} />
                                 </div>
                              ...
                              {!isViewerMode ? (
                                 ...
                                 <>
                                     <button 
                                         ...
                                         className={`h-11 px-6 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-all border flex-1 md:flex-auto ${
                                             isFollowing 
                        ...
                        <div key={service.id} className="glass-panel p-6 rounded-xl border border-white/5 hover:border-primary/30 transition-all group hover:shadow-[0_0_30px_rgb(var(--primary)/0.1)] relative overflow-hidden flex flex-col">
                            <div className="flex justify-between items-start mb-6 relative z-10">
                                ...
                                <div className="flex flex-col items-end">
                                    <span className="text-2xl font-bold text-white font-mono tracking-tight">${service.price}</span>
                                    <span className="text-[10px] text-neutral-500 font-mono uppercase bg-neutral-900 px-1.5 rounded">Starting at</span>
                                </div>
                            </div>
