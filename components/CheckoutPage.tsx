...
import { CreditCard, Bitcoin, Lock, CheckCircle, Copy, QrCode, AlertTriangle, ShieldCheck, ArrowRight } from 'lucide-react';
        ...
        return (
            <div className="w-full max-w-3xl mx-auto py-20 px-6 animate-in fade-in duration-500 text-center">
                ...
                <div className="bg-[#0a0a0a] border border-neutral-800 rounded-xl p-8 max-w-md mx-auto">
                    <p className="text-xs font-mono text-neutral-500 uppercase mb-4">Order #ORD-2025-X99</p>
                    ...
                    <button onClick={() => window.location.reload()} className="w-full py-3 bg-white/5 hover:bg-white/10 text-white font-bold rounded-lg border border-white/10 transition-colors">
                        Return to Dashboard
                    </button>
                </div>
            </div>
                        ...
                        <div className="space-y-4 mb-6 max-h-[300px] overflow-y-auto custom-scrollbar">
                            {MOCK_CART.map(item => (
                                <div key={item.id} className="flex gap-3">
                                    <div className="w-12 h-12 bg-neutral-800 rounded border border-white/10 overflow-hidden shrink-0">
                                        <img src={item.image} className="w-full h-full object-cover" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h4 className="text-xs font-bold text-white truncate">{item.title}</h4>
                                        <p className="text-[10px] text-neutral-500 truncate">{item.type}</p>
                                    </div>
                                    <div className="text-xs font-mono font-bold text-white">${item.price}</div>
                                </div>
                            ))}
                        </div>
